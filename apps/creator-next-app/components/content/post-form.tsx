    ...
    >
            ...
            {uploadedMediaFiles.length > 0 ? (
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  {uploadedMediaFiles.map((media, index) => (
                    <div
                      key={media.id || index}
                      className="relative group overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 aspect-square"
                    >
                      {/* Image Display */}
                      {media.type === "image" && (
                        <img
                        ...
                        <div className="relative w-full h-full">
                          ...
                          <div className="absolute inset-0 flex items-center justify-center bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity">
                            <div className="bg-white/90 dark:bg-gray-900/90 rounded-full p-3">
                              <Icons.play className="w-5 h-5 text-gray-900 dark:text-white" />
                            </div>
                          </div>
                          {/* Duration Badge */}
                          {media.duration && (
              ...
              >
                ...
                <div className="mt-4 text-sm text-gray-600 dark:text-gray-300">
                  <span className="font-medium text-primary-600 dark:text-primary-400">{t("content.uploadFiles")}</span>
                </div>
      ...
      <div className="bg-gray-50 dark:bg-gray-800 py-3 flex justify-end gap-3 px-6 -mx-6 -mb-6">
        ...
        <Button type="submit" disabled={!form.state.canSubmit || loading}>
          {loading && <Icons.spinner className="animate-spin h-4 w-4 mr-2" />}
          {getSubmitButtonText()}
        </Button>
      </div>
    </form>
