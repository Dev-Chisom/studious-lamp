		...
		<div>
			...
			<div class="mb-8">
				...
				<div
					...
					<NuxtLink to="/explore" class="btn-primary mt-4">{{ $t('subscriptions.noActiveSubscriptions.cta') }}
					</NuxtLink>
				</div>

				<div v-else class="grid gap-6 md:grid-cols-3">
					<div
						v-for="subscription in activeSubscriptions" :key="subscription.id"
						class="bg-white dark:bg-gray-900 rounded-lg shadow-sm overflow-hidden">
						<div class="p-6">
							<div class="flex items-center space-x-4">
								<div class="avatar h-16 w-16">
									<img
										:src="subscription.creator.profileImage" :alt="subscription.creator.displayName"
							...
							<div class="mt-4 space-y-2">
								<div class="flex justify-between text-sm">
									...
									<span class="font-medium">{{ subscription.plan === 'monthly' ? $t('common.monthly') :
										$t('common.yearly') }}</span>
								</div>

								<div class="flex justify-between text-sm">
									<span class="text-gray-500 dark:text-gray-200">{{ $t('subscriptions.price') }}</span>
									<span class="font-medium">${{ subscription.price }}/{{ subscription.plan === 'monthly' ?
										$t('common.month') : $t('common.year') }}</span>
								</div>
			...
			<div v-if="expiredSubscriptions.length > 0">
				...
				<div class="grid gap-6 md:grid-cols-2">
					<div
						v-for="subscription in expiredSubscriptions" :key="subscription.id"
