					...
					<div class="mt-4">
						<div class="flex items-center text-sm">
							...
							<span class="ml-2 text-gray-500 dark:text-gray-200 dark:text-gray-400">{{
								$t('earnings.stats.fromLastMonth') }}</span>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="p-6">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<div class="space-y-4">
							<div v-for="item in revenueByType" :key="item.name" class="flex items-center sm:flex-col md:flex-row">
								<div class="w-full bg-gray-200 rounded-full h-2.5">
									<div
										class="h-2.5 rounded-full" :class="`bg-${item.color}-500`"
										:style="{ width: `${item.percentage}%` }" />
								</div>

								<div class="ml-4">
									<p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{
										$t(`earnings.revenue.types.${item.key}`) }}</p>
								</div>
					...
					<div>
						...
						<div class="space-y-4">
							<div
								v-for="item in revenueBySubscription" :key="item.name"
								...
								<div class="ml-4">
									<p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{
										$t(`earnings.revenue.subscriptionTypes.${item.key}`) }}</p>
								</div>
		...
		<div class="bg-white dark:bg-gray-900 shadow-sm rounded-lg">
			<div class="p-6">
				<div class="sm:flex sm:items-center sm:justify-between mb-6">
					...
					<div class="mt-3 sm:mt-0"><button class="btn-outline">{{ $t('earnings.transactions.downloadCsv') }}</button>
					</div>
				</div>
				...
				<div class="overflow-x-auto">
					<table class="min-w-full divide-y divide-gray-200">
						<thead>
							<tr>
								<th
									v-for="(label, key) in transactionHeaders" :key="key"
								...
								<td class="px-6 py-4 whitespace-nowrap">
									<div class="flex items-center">
										<div class="avatar h-8 w-8">
											<img
												:src="transaction.customer.avatar" :alt="transaction.customer.name"
