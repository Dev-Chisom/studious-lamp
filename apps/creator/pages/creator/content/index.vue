<template>
  <div class="max-w-6xl mx-auto">
                  ...
                  <tr v-for="item in filteredContent" :key="item._id" class="hover:bg-gray-50 dark:hover:bg-gray-800">
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                      <div class="flex items-center gap-2">
                        <div v-if="item.mediaFiles && item.mediaFiles.length > 0" class="h-10 w-10 flex-shrink-0">
                          <img class="h-10 w-10 rounded-lg object-cover" :src="item.mediaFiles[0] || '/placeholder.png'"
                            :alt="t('mediaPreview')" />
                        </div>
                        <div>
                    ...
                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-400">
                      <span class="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium" :class="{
                    ...
                    <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                      <div class="flex justify-end gap-2">
                        <button
                          ...
                          @click="editContent(item)" :title="$t('content.management.list.edit')">
                        ...
                        <button class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 p-1"
                          @click="confirmDelete(item)" :title="$t('content.management.list.delete')">
    ...
    <Pagination v-if="pagination.pages > 1" :currentPage="page" :perPage="limit" :totalItems="pagination.total"
      :perPageOptions="[10, 20, 50, 100]" :showEndButtons="true" :showPerPage="true" :showRange="true"
      @update:currentPage="val => { page = val }" @update:perPage="val => { limit = val }" />
