<template>
	<div class="container mx-auto max-w-6xl">
		<div>
			<h1 class="text-2xl font-bold mb-6 dark:text-white">{{ $t('messages.title') }}</h1>
				...
				<div class="grid grid-cols-1 md:grid-cols-3">
					...
					<div class="border-r border-gray-200 dark:border-gray-700">
						<div class="p-4 border-b border-gray-200 dark:border-gray-700">
							<form-input
								v-model="searchQuery" :placeholder="$t('messages.search')" icon="lucide:search"
						...
						<div class="divide-y divide-gray-200 dark:divide-gray-700 max-h-[600px] overflow-y-auto">
							<div
								...
								class="p-4 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer" :class="{
						...
						<template v-if="selectedChat">
							...
							<div class="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
								<div class="flex items-center space-x-3">
									<div class="avatar h-10 w-10">
										<img
											:src="selectedChat.user.avatar" :alt="selectedChat.user.name"
							...
							<div class="flex-1 overflow-y-auto p-4 space-y-4 dark:bg-gray-900">
								<div
									v-for="message in selectedChat.messages" :key="message.id" class="flex"
									:class="message.isSelf ? 'justify-end' : 'justify-start'">
									<div
										class="max-w-[70%] rounded-lg px-4 py-2" :class="message.isSelf
										...
										<div
											...
											<template v-if="Array.isArray(message.media)">
												<div v-for="(media, idx) in message.media" :key="idx">
													<img
														v-if="media.type === 'image'" :src="media.url" :alt="message.content"
														class="rounded-lg max-h-48 cursor-pointer dark:border dark:border-gray-700"
														@click="openMediaPreviewForChat(media, message.id)" />
													<video
														v-else-if="media.type === 'video'" :src="media.url" controls controlsList="nodownload"
											...
											<template v-else>
												<img
													v-if="message.media.type === 'image'" :src="message.media.url" :alt="message.content"
													class="rounded-lg max-h-48 cursor-pointer dark:border dark:border-gray-700"
													@click="openMediaPreviewForChat(message.media, message.id)" />
												<video
													v-else-if="message.media.type === 'video'" :src="message.media.url" controls
										...
										<div class="flex items-center justify-end mt-1 space-x-1">
											...
											<!-- Read receipt -->
											<Icon
												v-if="message.isSelf" :name="message.isRead ? 'lucide:check-check' : 'lucide:check'"
												class="h-4 w-4" :class="message.isRead ? 'text-accent-500' : 'opacity-70'" />
										</div>
									</div>
								</div>
								...
								<form class="space-y-4" @submit.prevent="sendMessage">
									...
									<div v-if="mediaPreview.length > 0" class="flex space-x-2 overflow-x-auto pb-2">
										<div v-for="(media, index) in mediaPreview" :key="index" class="relative flex-shrink-0">
											<img
												v-if="media.type === 'image'" :src="media.preview"
												class="h-20 w-20 object-cover rounded-lg dark:border dark:border-gray-700" />
											<video
												v-else-if="media.type === 'video'" :src="media.preview" controlsList="nodownload"
												class="h-20 w-20 object-cover rounded-lg dark:border dark:border-gray-700" />
											<button
									...
									<div class="flex space-x-2">
										<form-input
											v-model="newMessage" :placeholder="$t('messages.typeMessage')"
											class="flex-1 dark:bg-gray-800 dark:text-white" @input="handleTyping" />

										<!-- Media upload buttons -->

										<div class="flex space-x-2">
											<input
												ref="imageInput" type="file" accept="image/*" multiple class="hidden"
		...
		<!-- Media preview modal -->
		<media-preview-modal
			v-if="mediaModal.isOpen" :is-open="mediaModal.isOpen" :media-items="mediaModal.mediaItems"
			:current-index="mediaModal.currentIndex" @close="closeMediaPreview"
