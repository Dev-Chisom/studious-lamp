<template>
  <div>
      ...
      <div class="relative aspect-video w-full bg-gray-100 dark:bg-gray-800 overflow-hidden">
        <template v-if="post.isPremium && !isSubscribed">
          <div class="absolute inset-0 z-10 flex flex-col justify-center items-center bg-white/70 dark:bg-gray-900/70">
            <button class="max-w-md rounded-full btn-primary text-white font-semibold py-4 px-6 text-lg transition-all"
      ...
      <div class="px-4 py-3">
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-center space-x-4">
            <button :disabled="post.isPremium && !isSubscribed" :class="[
        ...
        <p class="text-sm font-semibold text-gray-900 dark:text-white mb-1">{{ formatNumber(post.likes) }} {{ t('likes')
          }}
        </p>
        ...
        <button v-if="(post.comments.length > 0 || localComments.length > 0) && !(post.isPremium && !isSubscribed)"
    ...
    <MediaPreviewModal v-if="showModal" :is-open="showModal" :media-items="mediaItemsForModal"
