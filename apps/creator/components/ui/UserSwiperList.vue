  ...
  <div>
    <div class="flex items-center justify-between mb-3">
      ...
      <div v-show="showNavigation" class="flex items-center gap-2">
        <button @click="slidePrev" :disabled="isBeginning" :class="[
        ...
        <button @click="slideNext" :disabled="isEnd" :class="[
          'p-1 rounded-full bg-gray-100 dark:bg-gray-800 hover:bg-primary-100 dark:hover:bg-primary-900/40 transition',
    ...
    <Swiper ref="swiperRef" :modules="[Pagination, Navigation, Keyboard]" :slides-per-view="1"
      ...
      <SwiperSlide v-for="(chunk, index) in chunkedUsers" :key="index">
        <div class="space-y-4">
          <div v-for="user in chunk" :key="user.id" class="relative flex flex-col justify-end rounded-xl overflow-hidden 
            aspect-[2/1] w-full group cursor-pointer transition">
            <NuxtLink to="/@user" class="flex-shrink-0">
              <img :src="user.banner || user.avatar" :alt="user.name"
                class="absolute inset-0 w-full h-full object-cover" loading="eager" />
              <div class="absolute bottom-0 left-0 right-0 h-1/2 bg-gradient-to-t from-black/60 via-black/40 to-transparent 
