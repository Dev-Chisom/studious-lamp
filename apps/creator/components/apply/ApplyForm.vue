<template>
	<Form
		v-slot="{ errors, meta, values, setFieldValue }" :validation-schema="applyFormSchema" :initial-values="initialValues"
		class="space-y-8" validate-on-input @submit="onSubmit">
		<!-- Basic Information -->
		<section>
			...
			<div class="space-y-6">
				<Field v-slot="{ field, meta }" name="displayName">
					<FormInput
						v-model="field.value" :label="t('apply.basicInfo.displayName')" name="displayName"
				...
				<Field v-slot="{ field, meta }" name="username">
					<FormInput
						v-model="field.value" v-bind="field" :label="t('apply.basicInfo.username')" name="username"
				...
				<Field v-slot="{ field, meta, handleChange }" name="bio">
					<div>
						...
						<textarea
							v-bind="field" rows="3" :placeholder="t('apply.basicInfo.bioPlaceholder')" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 focus:border-primary-500 focus:ring-primary-500"
				...
				<Field v-slot="{ value = [], handleChange }" name="categories">
					<div>
						...
						<div class="flex flex-wrap gap-2">
							<button
								v-for="cat in availableCategories" :key="cat" type="button"
								...
								{{ cat }}
								<Icon
									v-if="value.includes(cat)" name="lucide:x" class="ml-1 h-4 w-4"
		...
		<section class="pt-8 border-t border-gray-200 dark:border-gray-700">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<Field v-slot="{ field }" name="social.facebook">
					<FormInput
						v-model="field.value" v-bind="field" :label="t('apply.social.facebook')" name="social.facebook"
				...
				<Field v-slot="{ field }" name="social.instagram">
					<FormInput
						v-model="field.value" v-bind="field" :label="t('apply.social.instagram')" name="social.instagram"
				...
				<Field v-slot="{ field }" name="social.twitter">
					<FormInput
						v-model="field.value" v-bind="field" :label="t('apply.social.twitter')" name="social.twitter"
				...
				<Field v-slot="{ field }" name="social.tiktok">
					<FormInput
						v-model="field.value" v-bind="field" :label="t('apply.social.tiktok')" name="social.tiktok"
			...
			<div class="space-y-6">
				<div class="mb-8">
					...
					<Field v-slot="{ field, meta }" name="monthlyPrice">
						<FormInput
							v-model="field.value" v-bind="field" type="number" :label="t('apply.pricing.monthlyPrice')"
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
					<div
						v-for="(options, cycle) in preferencesByCycle" :key="cycle"
						...
						<FormInput
							:model-value="periodPrices[cycle]" type="number"
		...
		<section class="pt-8 border-t border-gray-200 dark:border-gray-700">
			<div class="flex items-start">
				<Field
					v-slot="{ field, errorMessage, meta }" name="acceptTerms" type="checkbox" :value="true"
					:unchecked-value="false">
					<div class="flex items-start mt-2">
						<div class="flex items-center h-5">
							<input
								id="accept-terms" type="checkbox" v-bind="field"
			...
			<Field
				v-slot="{ field, errorMessage, meta }" name="confirmAge" type="checkbox" :value="true"
				:unchecked-value="false" class="mt-4">
				<div class="flex items-start mt-2">
					<div class="flex items-center h-5">
						<input
							id="confirm-age" type="checkbox" v-bind="field"
							class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500" />
					</div>
					<div class="ml-3 text-sm">
		...
		<div class="pt-6 flex justify-end">
			<button
				type="submit" :disabled="!meta.valid || apiLoading"
